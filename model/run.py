...
from utils_own.utils import save_reconstructions
from utils_own.utils import (seed_everything, worker_init_fn)
...
class DataTransform:
    ...
    def __call__(self, kspace, fname, slice):
        ...
        return  self.mask, under_kspace_tensor, torch.from_numpy(under_img),torch.from_numpy(target_img), fname, slice


def create_data_loaders(args):
    mask = sio.loadmat("./mask/%s/%s/%s_256_256_%d.mat" % (args.data, args.mask, args.mask, args.rate))['Umask']
    data = SliceData(
        root = args.data_path/f'Test',
...
def run_unet(args, model, data_loader):
    ...
    with torch.no_grad():
        for data in data_loader:
            mask, under_kspace_tensor, under_img_tensor, target_img_tensor, fnames, slices = data
            ...
            recons, recons_k = model(mask, under_kspace_tensor.permute(0,3,1,2), under_img_tensor.permute(0,3,1,2))
            recons = (recons ** 2).sum(dim=-1).sqrt()
            recons = (recons-recons.min())/(recons.max()-recons.min())*255 - 128
